...
namespace TeamBuilder.Dialogs
{
    public class AddTeamMemberDialogViewModel : BaseViewModel
    {
        ...
        public AddTeamMemberDialogViewModel(IDialogAware dialogAware, IDialogParameters parameters)
        {
        ...
        private async Task ExecuteConfirmCommand()
        {
            try
            {
                ...
                {
                    await ShowAlertDialogAsync("Error", validationResult.Error);
                    return;
                }

                if (_newMemberViewModels.Any(x => x.Name == Name))
                {
                    await ShowAlertDialogAsync("Error", $"Team member with the same name ({Name}) has already been added to the list.");
                    return;
                }
