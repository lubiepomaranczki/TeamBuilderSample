...
namespace TeamBuilder.TeamMembers.Application.AddTeamMembers
{
    public class AddTeamMembersViewModel : BaseViewModel
    {
        ...
        public AddTeamMembersViewModel(ITeamMembersRepository teamMembersRepository)
        {
        ...
        private async Task ExecuteAddNewTeamMemberCommand()
        {
            try
            {
                ...
                var result = (IDialogParameters) await App.Current.MainPage.ShowPopupAsync(new AddTeamMemberDialog(dialogParameters));
            ...
            {
                await ShowAlertDialogAsync("Error", ex.Message);
            }
        }
        ...
        private async Task ExecuteSaveCommand()
        {
            try
            {
                ...
                {
                    await ShowAlertDialogAsync("Warning", "No team members were added.");
                    return;
                }
                ...
                {
                    await ShowAlertDialogAsync("Error", currentTeamMembersResult.Error);
                    return;
                }
                ...
                foreach (var currentTeamMember in currentTeamMembersResult.Value)
                {
                    ...
                    {
                        await ShowAlertDialogAsync("Error", $"Team member with the same name ({currentTeamMember.Name}) has already been added to the list.");
                        return;
                    }
                }
                ...
                foreach (var teamMemberViewModel in NewMemberViewModels)
                {
                    ...
                    {
                        await ShowAlertDialogAsync("Error", $"Error adding team member {teamMemberViewModel.Name}:\r\n{addTeamMemberResult.Error}");
                    }
                }
        ...
        private async Task ExecuteRemoveCommand()
        {
            try
            {
                ...
                var questionResult = await ShowQuestionDialogAsync("Question", "Are you sure you want to remove selected team member from the list?");
        ...
        private async Task ExecuteCancelCommand()
        {
            try
            {
                var questionResult = await ShowQuestionDialogAsync("Question", "Are you sure you want to cancel?");
                if (questionResult == false)
                    return;
